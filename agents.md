...
* Publicado en `main`: commit **`b08db41`**, despliegue Hostinger **`34518129388` success**. Verificación HTTP posterior: home sirve `style.css?v=4.2`, `main.js?v=3.9`, que carga `upload.js?v=5`; CSP pública incluye `https://scripts.clarity.ms`.
* El monitor de Supabase pasó de lunes/jueves a diario a las 12:00 UTC. El plan Pro sigue siendo la única garantía del proveedor contra pausa automática por inactividad; no se inicia migración a Firebase.
* Formulario: selector `Tipo de servicio` con esquema oscuro y contraste explícito; Clarity deja de bloquearse por CSP. Ante caída de red de Storage/base de datos, el formulario intenta entregar datos por EmailJS, y solo confirma éxito si ese respaldo responde; si un archivo no llegó, prepara el mensaje de WhatsApp con el nombre del adjunto.
* Framer: Omar abrió el dashboard para retirar el sitio heredado `rtmx.framer.website`. Se indicó despublicar si quiere conservarlo o eliminar el proyecto desde el menú `…` de su tarjeta. La eliminación queda bajo control de Omar en Framer; no se comprobó aún su ejecución ni la desindexación posterior.
...
* Formulario principal: se retiró `novalidate` para que la validación nativa sea el respaldo si el stack diferido de Supabase no carga todavía. Cuando `upload.js` está listo, cada campo obligatorio muestra un mensaje visible junto al campo y `aria-invalid`; al corregir el campo, el mensaje desaparece. No se cambió qué datos son obligatorios ni se realizó ningún envío.
* Adjuntos: zona de carga ahora tiene semántica de botón, nombre accesible, ayuda asociada, foco visible y abre el selector con Enter/Espacio además de clic. Los botones de eliminar archivo también tienen foco visible. No se probaron archivos reales para no subir datos ni abrir selector del sistema.
* Contenido con `.reveal`: visible por defecto. Solo tras ejecutar `main.js` se añade clase `js` que habilita la animación existente; si el JS falla, el contenido queda disponible. Cache-bust preparado: `style.css?v=4.1`, `main.js?v=3.7`, carga dinámica `upload.js?v=3`; CSS y JS independientes de Lonas permanecen sin cambio.
* Validación local: sintaxis JS, `git diff --check`, auditoría de 40 HTML/109 JSON-LD, accesibilidad/semántica de campos y versiones. Navegador local: formulario con servicio Alucobond preseleccionado, submit vacío bloqueado sin envío y errores visibles para nombre/correo; al llenar nombre se retira solo su aviso. Zona de archivo expuesta como botón con `tabindex=0`. Evidencia `scratch/form-resilience-verification-2026-09-10/`; verificador `scratch/verify-form-resilience-2026-09-10.cjs`.
* Falta: prueba autorizada de envío real desde la red de Omar y revisión de adjunto real; revisión completa de rendimiento/consentimiento y contenido comercial/casos siguen en backlog. No se puede confirmar indexación en GSC sin acceso.
* **Corrección móvil posterior:** Omar reportó que al enfocar campos el navegador regresaba al hero y que «Obra y Mtto» se desplazaba a la izquierda dentro del menú. Confirmado en local que `.nav-dropdown:hover .nav-dropdown-menu` de desktop, por especificidad, conservaba `translateX(-50%)`: el enlace iniciaba en `x=-123.5` a 375 px. La regla móvil específica ahora fuerza `transform:none`/`left:auto`; comprobado `x=32`, igual que la lista del menú, sin overflow. Los inputs, select y textarea se fijaron en **16px** en el breakpoint móvil para evitar el autozoom de Safari/iOS al enfocarlos, causa probable del salto vertical que Chrome desktop no reproduce con teclado virtual. Comprobado CSS calculado 16px. Confirmar en el teléfono de Omar tras recibir caché nueva.
* **Verificación pública:** home, `style.css?v=4.1`, `main.js?v=3.7` y `upload.js?v=3` responden 200 y coinciden con los archivos locales normalizando saltos de línea; evidencia `scratch/part3-live-*.txt`. Los directorios locales sin versionar se excluyeron del commit.
...
* Menú desktop visible en tres columnas; menú móvil de una columna desplazable. Verificado Escape: primero cierra Servicios y enfoca el resumen; segundo cierra menú móvil y enfoca `menu-toggle`. Hero home revisado a 360 px, menú a 390 px, Gran Formato a 768/1024/1282/1440 px.
* **Fallo encontrado y corregido:** cabeceras de servicios con diez enlaces partían textos a 1024 px y cerca del límite desktop. Solo las reglas de navegación compacta se extienden de 968 a 1280 px (el resto de estilos responsive sigue en 968); entre 1281 y 1500 px se reduce el espacio entre enlaces y se evita su partición. Confirmado a 1282 px: diez elementos en una sola línea, cabecera de ~83 px, sin desbordamiento horizontal. A 1024/768 px aparece menú compacto. CSS sigue v4.0 porque este bloque aún no se publica.
* Recorrido real Alucobond ES → CTA → home: URL conserva `servicio=alucobond`, campo seleccionado correctamente. EN → CTA → home EN: WhatsApp y correo contienen el servicio de fachada instalada. Sin abrir aplicaciones de mensajería ni enviar formularios.
* Se revisaron las seis FAQ EN: pregunta de precio Alucobond abierta con Enter y respuesta visible; primera pregunta de las cinco páginas restantes abierta por clic, sin desbordamiento horizontal a 390 px. Capturas visuales de Alucobond/Toldos confirman legibilidad. Express EN conserva navegación y selector ES visibles a 390 px. Esto no equivale a auditoría exhaustiva de accesibilidad ni a verificar todos los dispositivos.

---
## 🛠️ Parte 2B en local — 10 de Septiembre, 2026 (navegación y contexto de cotización)

* Omar pidió continuar. Se añadieron destinos comerciales directos en el menú de las **36 páginas con navegación común**, mediante `details/summary` nativo y tres grupos: impresión, rótulos/anuncios, fachadas/protección. Versiones EN con etiquetas traducidas y aviso de los destinos solo en español. Express y Lonas conservan sus navegaciones independientes; el menú «Más» no se rediseñó.
* Panel en tres columnas en desktop y una columna dentro del menú móvil; scroll disponible. `main.js` cierra el directorio al pulsar fuera, salir con Tab o Escape. Primer Escape cierra el directorio y devuelve foco a su resumen; el siguiente cierra el menú móvil y devuelve foco al botón de menú. No se afirma auditoría completa de accesibilidad ni prueba visual.
* **88 enlaces de contacto/cotización** desde servicios transportan `?servicio=...` hacia la home correspondiente. Nuevo `/quote-context.js?v=1`, cargado solo en home ES/EN: lista permitida de diez servicios, ignora valores desconocidos/repetidos y conserva una selección ya hecha. ES preselecciona la opción del formulario; EN no tiene formulario, por lo que personaliza enlaces de WhatsApp y correo dentro de `#contact`. No almacena datos ni envía mensajes. La calculadora específica de placas mantiene su mensaje con color/cantidad.
* Gran Formato ES: H1 y explicación inicial orientados a aplicaciones/materiales, sección que distingue cotizador de Lonas (medidas/mínimo) y Express (fecha/disponibilidad). No se fusionaron URLs ni se modificaron precios. La revisión editorial integral de Express/Lonas y los casos de la home siguen pendientes.
* Cache-bust en el mismo bloque: **`style.css?v=4.0`, `main.js?v=3.6`**; CSS y JS independientes de Lonas conservados. Sitemap actualizado para páginas de servicio con cambio del recorrido de cotización; fechas de páginas con solo menú/cache-bust conservadas.
* **Validación:** auditoría estática de 40 HTML y 109 JSON-LD sin fallos de enlaces/anclas/hreflang, duplicados exactos o H1; 36 directorios; 88 enlaces de contexto válidos. Pruebas de `quote-context.js` en VM con DOM simulado: diez servicios ES y diez EN, seis entradas inválidas/ausentes/repetidas, selección anterior preservada, destinos WhatsApp/correo correctos y teléfono intacto. Las 25 FAQ de 2A siguen coincidiendo con contenido visible. Sintaxis JS y `git diff --check` correctos. Evidencia `scratch/seo-part2b-verification-2026-09-10/`, verificador `scratch/verify-seo-part2b.cjs`.
* **Estado:** partes 2A y 2B locales, sin commit/push/deploy. `cua.getState()` volvió a devolver apps/navegadores vacíos; falta validación visual desktop/móvil y teclado en navegador real. No hubo envíos de formulario. Próximo bloque: revisión visual cuando haya navegador, casos/contexto de galería y robustez de formulario/contenido sin JS según backlog; Framer/GSC requieren acceso.

---
## 🛠️ Parte 2A en local — 10 de Septiembre, 2026 (servicios EN)

* Omar pidió continuar por partes. Siguiente bloque implementado: contenido útil y preguntas visibles en `alucobond-en.html`, `anuncios-luminosos-en.html`, `gran-formato-en.html`, `letras-3d-en.html`, `neon-flex-en.html` y `toldos-en.html`. **Cambios locales, sin commit/push ni despliegue de este bloque.**
* Las 25 preguntas antes exclusivas de JSON-LD tienen ahora respuestas visibles en `details/summary`, operables sin JS y con los estilos FAQ existentes. Se revisaron las respuestas para explicar condiciones de instalación, materiales, archivos y plazos orientativos; coincidencia exacta entre texto visible y FAQPage. No se promete que el marcado produzca resultados enriquecidos.
* Cada servicio incorpora qué enviar para cotizar y enlaces comerciales pertinentes. Alucobond EN muestra el precio confirmado desde $990 MXN/m² con material e instalación y condiciones; enlaza al portafolio existente identificándolo como experiencia del instalador aliado en México y a la venta separada de placas de $1,850. Los destinos en español se indican explícitamente.
* Gran Formato EN prioriza aplicaciones/materiales y distingue el cotizador de Lonas (mínimo facturable 1 m² por pieza) de Express (disponibilidad por confirmar). Diferenciación completa de páginas ES, navegación comercial global y selección de servicio al cotizar quedan para el bloque 2B; no se consideran cerradas.
* Sitemap: seis fechas EN corresponden al 10-sep (Gran Formato ya tenía esa fecha). CSS/JS sin cambios; versiones permanecen `style.css?v=3.9` y `main.js?v=3.5`.
* Validación local: 40 HTML con un H1, contenedores comprobados balanceados, JSON-LD válido, 25 FAQ con igualdad exacta entre schema y contenido visible, sin enlaces/anclas rotos, errores de mayúsculas/hreflang ni títulos/descripciones duplicados exactos. Fechas de sitemap comprobadas y `git diff --check` sin errores. Evidencia separada en `scratch/seo-part2a-verification-2026-09-10/`; verificador `scratch/verify-seo-part2a.cjs`. No se hizo prueba visual, envío de formulario ni comprobación pública de estos cambios locales.

---
## ✅ Parte 1 desplegada — 10 de Septiembre, 2026 (correcciones técnicas y navegación)

* Omar pidió hacer las correcciones por partes y posteriormente autorizó push. Se implementó y publicó el primer bloque: redirecciones, navegación móvil, enlaces internos, sitemap y correcciones acotadas de datos estructurados. **Commit `9026708` en `main`; despliegue `34498382701` terminado en `success`.** La auditoría previa permanece como registro del estado observado antes de corregir.
* **Verificación en producción:** home, Express ES/EN, ambos CSS modificados, placas y sitemap responden 200 y coinciden íntegramente con los archivos locales normalizando saltos de línea. `/index.html?utm_source=seo-part1-check` devuelve 301 a la raíz conservando parámetros; el índice de Express EN con parámetros llega a su URL de directorio en una redirección y termina en 200. Evidencia HTTP en `scratch/part1-live-*.txt`. Carpetas locales no versionadas excluidas del commit.
* `.htaccess`: condiciones `THE_REQUEST` para índices de raíz/subcarpetas reconocen `?` además del separador de la petición. Destinos sin query conservan los parámetros según Apache. Se mantiene la condición sobre la petición original para no redirigir índices resueltos internamente y evitar bucles. No se alteró la regla HTTPS por las respuestas intermitentes observadas en la auditoría: falta verificar CDN/origen tras deploy.
* `style.css`: reglas móviles de menú acotadas a `#main-nav`, evitando ocultar/fijar `<nav class="breadcrumbs">`. Menú alineado desde arriba y desplazable verticalmente para pantallas bajas. **Cache-bust raíz `style.css?v=3.9`** en las páginas versionadas que lo cargan. El CSS independiente `lonas-cancun/style.css?v=3.8` se conserva; `main.js?v=3.5` no cambió.
* Express ES/EN: nuevo `/express/navigation.css?v=1` compartido. Hasta 1023 px, los cuatro enlaces existentes pasan a una fila propia visible, incluido el idioma, sin JS adicional. Se reserva espacio bajo la cabecera y al navegar a secciones, se mantiene foco visible y se ajusta el CTA para que quepa en pantallas estrechas. CTA del header con nombre accesible cuando su texto se oculta. Dimensiones reales añadidas a las seis imágenes estáticas de ambas páginas (logo 1600×323, hero 1600×900).
* Enlaces contextuales: `/en/` y `gran-formato-en.html` → Express EN; `control-solar.html` → artículo control solar vs polarizado; anuncios luminosos y letras 3D → guía de letreros. Express EN pasa de 1 a 3 páginas entrantes; artículo control solar de 1 a 2; artículo letreros de 1 a 3. No se rediseñó todavía el menú comercial global.
* Schema: home ES incorpora los perfiles oficiales de Instagram/Facebook ya enlazados. Placas declara la marca **Tach-Panel** según producto confirmado del handoff, vendedor Rotulate y tres fotos reales disponibles; marca también visible en especificaciones. Se retiró el `aggregateRating` antiguo **5.0/4** de la home EN, que no coincidía con la calificación visible **4.7/30+**. No se inventó una calificación nueva ni se modificaron precios/disponibilidad.
* Sitemap: fechas actualizadas de las páginas con cambios en este bloque; se corrigen retrospectivamente Alucobond a 9-sep y Lonas a 7-sep. Se conservan las fechas del resto de páginas que únicamente reciben cache-bust.
* **Validación local:** 40 HTML versionados balanceados en los contenedores comprobados; 109 JSON-LD parsean; 38 destinos de sitemap con canonical propio y accesibles a máximo 2 enlaces; sin duplicados exactos de títulos/metas, destinos HTML/anclas rotos ni errores de reciprocidad hreflang. **15 casos de patrones/destinos de redirección** pasan (GET/HEAD, parámetros, subcarpetas y prevención de bucles). `git diff --check` sin errores. Evidencia en `scratch/seo-part1-verification-2026-09-10/`; scripts `scratch/verify-seo-part1-2026-09-10.cjs` y auditor reutilizado con directorio separado, conservando evidencia anterior.
* **Límites pendientes:** `cua.getState()` sigue sin navegadores/apps. No se pudo validar visualmente a 360/390/768/1024 px. Los 15 casos locales validan patrones y destinos; las comprobaciones HTTP de producción arriba confirman raíz y subcarpeta con parámetros. Revisión visual pendiente; Framer/GSC siguen requiriendo acceso. No hubo envíos del formulario.
* **Parte 2 pendiente:** completar contenido de servicios EN y FAQ solo en código, diferenciar Lonas/Express/Gran Formato, casos, navegación comercial directa y conservar servicio al cotizar. Robustez sin JS y mejoras de formulario más amplias se mantienen en el backlog. Los cambios y la documentación de esta parte no deben mezclarse accidentalmente con carpetas locales sin versionar.

---
## 📅 Revisión — 10 de Septiembre, 2026 (indexabilidad, navegación y sitelinks)

* Omar mostró una captura de sitelinks de Hiplot y pidió revisar a fondo indexación, estructura y navegación. Auditoría detallada en [docs/auditoria-indexacion-navegacion-2026-09-10.md](docs/auditoria-indexacion-navegacion-2026-09-10.md). **Solo diagnóstico/documentación: no se modificó código público, no hubo commit/push/deploy ni envíos de formulario.**
* Verificado: **40 HTML versionados**, **38/38 URLs del sitemap HTTP 200**, canonical propio y HTML igual al repositorio (normalizando saltos de línea); **109 JSON-LD parsean**; sin títulos/metas duplicados exactos, enlaces HTML internos rotos o retornos hreflang ausentes. Todas las URLs del sitemap se alcanzan a un máximo de 2 enlaces desde la home. **129/129 recursos estáticos detectados en HTML/CSS responden 200** por HEAD. Los conteos de 41 HTML previos incluían contenido local fuera del inventario versionado.
* **Límite de indexación real:** no hay navegador conectado ni conector GSC. `search_info/` cubre 30-may–29-ago y `seo_performance/` 25-mar–24-jun; son rendimiento histórico, no cobertura actual. No se certifica indexación actual, canonical elegido por Google, SERP localizada, visual móvil/desktop o Core Web Vitals. Los CSV ya registran impresiones de las páginas EN que en julio estuvieron pendientes: no repetir aquel estado como vigente.
* **Framer antiguo activo:** GET de `https://rtmx.framer.website/` → 200, canonical propio, sin noindex, robots permite rastreo; también aparece en búsqueda pública. Omar preguntó cómo eliminarlo; se le explicó el flujo documentado por Framer Dashboard → Archive → Delete (permanente y despublica la web). **No se eliminó ni se cambió Framer.** Retirarlo/consolidarlo sigue pendiente; no está demostrado que cause una penalización o impida sitelinks.
* **Redirecciones:** primer barrido de HTTP home e `/index.html` dio 200; repeticiones independientes con `Cache-Control: no-cache` dieron 301 correcto. No atribuir causa definitiva al CDN. `/index.html?…` y `/express/en/index.html?…` dieron 200: las condiciones `THE_REQUEST` de `.htaccess` esperan espacio después de `index.html` y no cubren `?`. Canonical correcto mitiga duplicados. WWW, lonas sin barra, índices de subcarpeta sin query, `/blog.html` y `/home-b2/` redirigen bien; `/sectores/` 410 y URL inexistente 404.
* **Navegación:** el selector móvil global `nav` en `style.css:2215` afecta también `<nav class="breadcrumbs">` (oculta/fija las migas; hallazgo de cascada, falta prueba visual). Express ES/EN oculta todo el nav en móvil sin alternativa, incluido selector de idioma. Menú global prioriza secciones de home sobre destinos comerciales. Express EN solo recibe enlace desde Express ES; dos artículos siguen enlazados solo desde el blog. Propuestas detalladas, sin implementación; se conserva el aplazamiento previo de accesibilidad de «Más».
* **Contenido/schema:** seis servicios EN tienen FAQPage con 25 preguntas no mostradas en el HTML visible y contenido comercial muy resumido. `Product.brand` de placas usa Rotulate pese a producto Tach-Panel según handoff; revisar fabricante/vendedor y añadir imagen real. `lastmod` de home y Alucobond no refleja los cambios significativos del 9-sep. No confundir parseo correcto con fidelidad semántica.
* **Corrección de referencia anterior:** Google retiró los resultados enriquecidos FAQ desde el **7-may-2026** y su documentación en junio ([changelog oficial](https://developers.google.com/search/updates#may-2026)). Sustituye la expectativa histórica de FAQ limitadas a ciertos sectores: mantener preguntas útiles para clientes, no invertir en FAQPage para obtener sitelinks.
* Evidencia y scripts reproducibles en `scratch/seo-audit-2026-09-10/` y `scratch/seo-audit-*.cjs`, sin versionar. No agregar carpetas locales por accidente. Pendientes inmediatos: acceso GSC para inspección actual, retirada de Framer por Omar, consolidación de variantes y correcciones de navegación; aprobación de implementación no inferida de esta auditoría.
