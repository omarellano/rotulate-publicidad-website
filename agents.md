...
---
## 🛠️ Parte 3 en local — 10 de Septiembre, 2026 (formulario y contenido sin JS)

* Mientras Omar revisa la versión antigua de Framer, se avanzó el siguiente bloque técnico. **Cambios locales, sin commit/push/deploy.**
...
* **Corrección móvil posterior, sin publicar:** Omar reportó que al enfocar campos el navegador regresaba al hero y que «Obra y Mtto» se desplazaba a la izquierda dentro del menú. Confirmado en local que `.nav-dropdown:hover .nav-dropdown-menu` de desktop, por especificidad, conservaba `translateX(-50%)`: el enlace iniciaba en `x=-123.5` a 375 px. La regla móvil específica ahora fuerza `transform:none`/`left:auto`; comprobado `x=32`, igual que la lista del menú, sin overflow. Los inputs, select y textarea se fijaron en **16px** en el breakpoint móvil para evitar el autozoom de Safari/iOS al enfocarlos, causa probable del salto vertical que Chrome desktop no reproduce con teclado virtual. Comprobado CSS calculado 16px. Pendiente verificar en el teléfono de Omar antes de publicar.
